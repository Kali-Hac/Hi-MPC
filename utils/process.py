...
def gen_train_data(dataset, split, time_step, nb_nodes, nhood, global_att, batch_size, view='', reverse='0', D=False, graph=False, norm=True, L3=True):
	...
	def cal_features_ave(X, Y, Z):
		...
		for i in range(X.shape[0]):
			...
			for j in range(X.shape[1]):
				sub_f = []
				# head height
				h_1 = (2 * Y[i, j, 3] - Y[i, j, 15] - Y[i, j, 19]) / 2
				# neck height
				h_2 = (2 * Y[i, j, 2] - Y[i, j, 15] - Y[i, j, 19]) / 2
				sub_f.append(h_1)
				sub_f.append(h_2)
				for a, b in [[4,2], [8,2], [1,8], [8,10], [4,6], [16,17], [13,12],[2,1],[16,12]]:
					sub_f.append(np.sqrt((X[i,j,a]-X[i,j,b])**2+(Y[i,j,a]-Y[i,j,b])**2+(Z[i,j,a]-Z[i,j,b])**2))
				eights.append(np.sqrt((X[i,j,16]-X[i,j,17])**2+(Y[i,j,16]-Y[i,j,17])**2+(Z[i,j,16]-Z[i,j,17])**2))
				nines.append(np.sqrt((X[i,j,13]-X[i,j,12])**2+(Y[i,j,13]-Y[i,j,12])**2+(Z[i,j,13]-Z[i,j,12])**2))
				ratio_1 = np.sqrt((X[i,j,2]-X[i,j,1])**2+(Y[i,j,2]-Y[i,j,1])**2+(Z[i,j,2]-Z[i,j,1])**2) / \
				          max(np.sqrt((X[i,j,16]-X[i,j,17])**2+(Y[i,j,16]-Y[i,j,17])**2+(Z[i,j,16]-Z[i,j,17])**2), 0.1)
				ratio_2 = np.sqrt((X[i,j,2]-X[i,j,1])**2+(Y[i,j,2]-Y[i,j,1])**2+(Z[i,j,2]-Z[i,j,1])**2) / \
				          max(np.sqrt((X[i,j,13]-X[i,j,12])**2+(Y[i,j,13]-Y[i,j,12])**2+(Z[i,j,13]-Z[i,j,12])**2), 0.1)
				sub_f.append(ratio_1)
				sub_f.append(ratio_2)
				ave_f.append(sub_f)
			ave_f = np.array(ave_f)
			# print (ave_f.shape)
			ave_f = np.mean(ave_f, axis=0)
			# print(ave_f.shape)
			# exit(1)
			ave_f = ave_f.tolist()
			f.append(ave_f)
		...
		for i in range(X.shape[0]):
			...
			for j in range(X.shape[1]):
				sub_ff = []
				for a, b in [[3, 2], [1, 2], [1, 0], [8, 2], [8, 9], [9, 10], [2, 4], [4, 5], [5, 6],
				             [16,0],[16,17],[17,19],[12,0],[12,13],[13,15]]:
					sub_ff.append(np.sqrt((X[i,j, a] - X[i, j,b]) ** 2 + (Y[i, j,a] - Y[i, j,b]) ** 2 + (Z[i,j, a] - Z[i,j, b]) ** 2))
				h = h_1 = (2 * Y[i, j, 3] - Y[i, j, 15] - Y[i, j, 19]) / 2
				sub_ff.append(h)
				ave_f.append(sub_ff)
			ave_f = np.array(ave_f)
			ave_f = np.mean(ave_f, axis=0)
	...
	if D:
		D_13_test, D_16_test = cal_features_ave(t_input_data_x, t_input_data_y, t_input_data_z)
	...
	X_train_d = np.concatenate([input_data_x.reshape([-1, time_step, nb_nodes, 1]),
								input_data_y.reshape([-1, time_step, nb_nodes, 1]),
								input_data_z.reshape([-1, time_step, nb_nodes, 1])], axis=-1).reshape([-1, nb_nodes, 3])
	X_test_d = np.concatenate([t_input_data_x.reshape([-1, time_step, nb_nodes, 1]),
								t_input_data_y.reshape([-1, time_step, nb_nodes, 1]),
								t_input_data_z.reshape([-1, time_step, nb_nodes, 1])], axis=-1).reshape([-1, nb_nodes, 3])
	X_train_d_all = []
	X_test_d_all = []
	for i in range(X_train_d.shape[0]):
		X_train_d_all.append(pdist(X_train_d[i]))
	for i in range(X_test_d.shape[0]):
		X_test_d_all.append(pdist(X_test_d[i]))
		...
		if graph:
			...
			if not L3:
				return X_train_J, 0, 0, 0, 0, y_train, X_test_J, 0, 0, 0, 0, y_test, \
					   adj_joint, biases_joint, 0, 0, 0, 0, 0, 0, 0, 0, nb_classes, X_train_d_all, X_test_d_all
			else:
				return X_train_J, X_train_P, X_train_B, 0, 0, y_train, X_test_J, X_test_P, X_test_B, 0, 0, y_test, \
					   adj_joint, biases_joint, 0, 0, 0, 0, 0, 0, 0, 0, nb_classes, X_train_d_all, X_test_d_all
	else:
		return X_train_J, X_train_P, X_train_B, X_train_H_B, X_train_In, y_train, D_13_train, D_16_train,\
			   X_test_J, X_test_P, X_test_B, X_test_H_B, X_test_In, y_test, D_13_test, D_16_test, \
			   adj_joint, biases_joint, adj_part, biases_part, adj_body, biases_body, adj_hyper_body, biases_hyper_body, adj_interp, biases_interp, nb_classes
"""
